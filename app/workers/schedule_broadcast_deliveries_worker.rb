...
class ScheduleBroadcastDeliveriesWorker < ApplicationWorker
  ...
  def create_batch(broadcast)
    ...
    batch.on(:complete, ScheduleBroadcastDeliveriesWorker::Callback, 'broadcast_id' => broadcast.id)
    batch
  end
  ...
  def schedule_broadcast_recipient_delivery(broadcast, recipient)
    ...
    ScheduleBroadcastRecipientDeliveryWorker.perform_async(broadcast.id, recipient.id)
  end
