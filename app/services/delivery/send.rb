...
class Delivery < ApplicationRecord
  class Send < ApplicationService
    ...
    def remove_recipient
      logger.warn("Delivery #{delivery.id} is for a non-corporation member: #{delivery.member.name}")
      RemoveMemberJob.perform_later(delivery.member_id)
      nil
    end
    ...
    def nonexistent_step?
      ...
      if delivery.step_id.nil?
        logger.info("Delivery #{delivery.id} is for a campaign step that no longer exists")
        false
      else
        true
      end
    ...
    def send_evemail
      logger.info("Sending EVEmail for delivery #{delivery.id}")
