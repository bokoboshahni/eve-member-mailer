...
class SyncCorporationMembersJob < ApplicationJob
  ...
  class Callback
    def on_complete(status, options)
      ...
      SyncCorporationListMembersJob.perform_later(options['corporation_id'])
    end
  end
  ...
  def perform(corporation_id, user_id) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
    ...
    if new_members.empty?
      Rails.logger.info("Syncing corporation list members for #{corporation.name}")
      SyncCorporationListMembersJob.perform_later(corporation_id)
      return
    end

    batch = Sidekiq::Batch.new
    batch.description = "Syncing new members for #{corporation.name}"
    batch.on(:complete, SyncCorporationMembersJob::Callback, 'corporation_id' => corporation.id)
    batch.jobs do
      corporation.members.kept.where(eve_character_id: old_members).each { |m| RemoveMemberJob.perform_later(m.id) }
      new_members.each do |id|
        Rails.logger.info("Syncing #{corporation.name} member #{id}")
        SyncMemberJob.perform_later(id, corporation.id)
      end
    end
