...
RSpec.describe CreateAuthorization, type: :service, vcr: true do
  ...
  context 'when the character is available' do
    ...
    let(:authorization) { character.authorizations.find_by(kind: 'esi') }
  ...
  context 'when the character is already associated with the user' do
    ...
    let(:character) { SyncESICharacter.new(uid).call }
  ...
  context 'when the character is associated with another user' do
    it 'raises an error' do
      ...
      expect { service.call }.to raise_error(CreateAuthorization::AlreadyInUseError)
    end
  end
end
